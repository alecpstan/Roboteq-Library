...
import RoboteqManager  # Assumed to be in the same directory
from commands import command, query, config  # Assumed to be in the same directory
...
def load_config():
    ...
    try:
        ...
        if "devices" not in config_data:
            print("Warning: config.json exists but has no 'devices' array. Expected format:")
        ...
        if not config_data["devices"]:
            ...
            print(f"Successfully loaded {len(config_data['devices'])} device(s) from config.json")
...
def save_config(config_data):
    ...
    if "devices" in config_data:
        ...
        config_data["devices"], removed_count = filter_duplicate_devices(config_data["devices"])
    ...
    print(f"Configuration saved to config.json with {len(config_data.get('devices', []))} device(s)")
...
def scan_tcp_devices():
    ...
    while True:
        ip_range = input("Enter IP range to scan (e.g., '192.168.1.1-10'): ").strip()
        ...
        if "-" in ip_range and ip_range.count(".") == 3:
            ...
            print("Invalid format. Please use format like '192.168.1.1-10'")
    
    # Implement using RoboteqManager's capabilities
    tcp_devices = []
    try:
        print(f"Scanning IP range: {ip_range}")
        tcp_devices = RoboteqManager.RoboteqDriver.scan_tcp(ip_range)
...
def scan_com_ports():
    ...
    com_devices = RoboteqManager.RoboteqDriver.scan_ports()
...
def select_devices(devices):
    ...
    while True:
        try:
            selections = input("\nEnter numbers of devices to connect (comma-separated) or 'all': ").strip()
...
def filter_duplicate_devices(devices):
    ...
    for device in devices:
        ...
        if device_type == "serial":
            ...
            print(f"Warning: Unknown device type '{device_type}' for {device.get('name', 'Unnamed')}")
            continue
    ...
    for device in devices:
        try:
            ...
            if device_type == "serial":
                ...
                connection = RoboteqManager.RoboteqDriver(
...
def main():
    ...
    if prompt_yes_no("Would you like to scan for TCP devices?"):
        ...
        if tcp_devices:
            ...
            if removed > 0:
                print(f"Note: {removed} duplicate device(s) were detected and will not be added")
