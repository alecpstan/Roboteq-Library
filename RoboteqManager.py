...
# ----------------------------------------------------------------------------------------------------
    ...
    def scan_tcp(host_range, port=9571, timeout=0.5):
        ...
        print(f"This will check {end - start + 1} addresses with a {timeout}s timeout each")
        ...
        for i in range(start, end + 1):
            ...
            if hosts_checked % progress_step == 0 or hosts_checked == 1 or hosts_checked == total_hosts:
                print(f"Checking host {host} ({hosts_checked}/{total_hosts}, {hosts_checked/total_hosts*100:.1f}%)")
    ...
    def scan_ports(timeout=0.5):
        ...
        if sys.platform.startswith('win'):
            ...
            print(f"Scanning Windows COM ports (COM1-COM256) for Roboteq controllers...")
        ...
        print(f"Found {len(ports)} potential ports to check with {timeout}s timeout each")
        ...
        for port in ports:
            ...
            # Show periodic progress
            if ports_checked % progress_step == 0 or ports_checked == 1 or ports_checked == total_ports:
                print(f"Checking port {port} ({ports_checked}/{total_ports}, {ports_checked/total_ports*100:.1f}%)")
        ...
        # Final summary
        print(f"Serial port scan complete. Found {len(available_ports)} Roboteq controller(s).")
        if available_ports:
            print("Available controllers:")
...
# ----------------------------------------------------------------------------------------------------
    ...
    def __init__(self, connection_type=CONN_SERIAL, com_port=None, host=None, tcp_port=9571, timeout=1, num_axis=1):
        """
    ...
    def run_command(self, cmd_str, cc=1, nn=None):
        ...
        if not hasattr(command, cmd_str):
            raise ValueError(f"Invalid command: {cmd_str}. Must be a command from RoboteqDriver.cmd")
    ...
    def run_query(self, qry_str, cc=1):
        ...
        if not hasattr(query, qry_str):
            raise ValueError(f"Invalid query: {qry_str}. Must be a query from RoboteqDriver.qry")
    ...
    def motor_config(self, config_str, cc=1, nn=None, set=False):
        ...
        if not hasattr(config, config_str):
            raise ValueError(f"Invalid configuration run_query: {config_str}. Must be from RoboteqDriver.config")
...
# ----------------------------------------------------------------------------------------------------
    ...
    def linear_configure(self, axis, mm_per_rev, min_mm=None, max_mm=None):
        ...
        if points_per_rev <= 0:
            raise ValueError("Invalid EPPR received from controller. Configure EPPR first using motor_config method.")
        
        # Validate min/max if provided
        if min_mm is not None and max_mm is not None:
            if min_mm >= max_mm:
                raise ValueError(f"min_mm ({min_mm}) must be less than max_mm ({max_mm})")
        ...
        print(f"  Points per revolution: {points_per_rev} (from controller EPPR configuration)")
    ...
    def linear_home_axis(self, axis, dio_pin, direction="rev", speed=100, deceleration=3000):
        """
        ...
        if axis not in self.linear_config or not self.linear_config[axis-1]["enabled"]:
            raise ValueError(f"Axis {axis} is not configured for linear movement. Call configure_linear first.")
    ...
    def linear_move_absolute_mm(self, axis, position_mm, wait=True, position_tolerance=25):
        """
        ...
        # Validate axis is configured for linear movement
        if axis not in self.linear_config or not self.linear_config[axis-1]["enabled"]:
            raise ValueError(f"Axis {axis} is not configured for linear movement. Call linear_configure first.")
        
        # Check if axis has been homed
        if not self.linear_config[axis-1]["homed"]:
            raise ValueError(f"Axis {axis} has not been homed. Call linear_home_axis first.")
        ...
        if not isinstance(position_mm, (int, float)):
            raise ValueError(f"Position must be a number or 'home', got {type(position_mm)}")
        ...
        if min_mm is not None and position_mm < min_mm:
            raise ValueError(f"Position {position_mm}mm is below minimum limit of {min_mm}mm")
        if max_mm is not None and position_mm > max_mm:
            raise ValueError(f"Position {position_mm}mm is above maximum limit of {max_mm}mm")
        ...
        while True:
            ...
            try:
                ...
                if time.time() - start_time > max_wait_time:
                    print(f"Warning: Move timeout after {max_wait_time} seconds. Target: {position_counts}, Current: {current_position}")
                    return response
                    
            except (ValueError, IndexError):
                print(f"Warning: Could not parse position response: {current_position_response}")
